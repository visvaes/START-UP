    ...
    <section id="services" className="bg-[#f3f4ee] py-12 md:py-16">
      <div className="mx-auto max-w-7xl px-6">
        ...
        <div className="mb-10 flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
          <h2 className="text-4xl font-black leading-[0.9] tracking-[-0.05em] text-[#111827] md:text-6xl">
            Engineering digital
            <span className="block text-[#9bbd2d]">solutions that scale</span>
          </h2>

          <a href="#contact" className="inline-flex items-center gap-2 self-start rounded-full bg-[#d7eb7a] px-5 py-3 text-sm font-semibold text-[#1f2937] shadow-lg shadow-[#d7eb7a]/30 transition hover:brightness-105">
        ...
        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          {services.map((s) => (
            <article key={s.id} className="group overflow-hidden rounded-[24px] border border-[#dfe6d4] bg-[#f8f9f5] p-4 shadow-[0_12px_30px_rgba(17,24,39,0.04)] transition hover:-translate-y-1 hover:border-[#bfd85d]">
              <div className="flex items-center justify-between pb-4">
                <h3 className="text-[1.7rem] font-semibold leading-none tracking-[-0.04em] text-[#111827] md:text-[1.9rem]">{s.title}</h3>
                <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full bg-[#d7eb7a] text-[#1d2a12] shadow-lg shadow-[#d7eb7a]/30">
                  <ArrowUpRight size={18} />
                </div>
              </div>

              <div className="mb-4 h-px w-full bg-[#dfe5d5]" />

              <p className="min-h-[60px] text-[0.97rem] leading-relaxed text-[#4b5563]">{s.desc}</p>
